...
QuadTree::QuadTree(Point p) : unique_ptr<Node>(new Node(p)) {}

QuadTree* QuadTree::Search(Point k) {
	QuadTree* loper = this;

	while (loper->get() != nullptr && (loper->get()->point.x != k.x && loper->get()->point.y != k.y)) {
		if (k.x <= loper->get()->point.x) {
			if (k.y <= loper->get()->point.y) {//NW
...
void QuadTree::Add(Point k) {
	QuadTree* plaats = Search(k);
	if (plaats->get() == nullptr) {
		*plaats = move(std::unique_ptr<Node>(new Node(k)));
	}
}
...
void QuadTree::Write(ostream & os) const
{
	if (*this != nullptr) {
